...
const UpdateDetails = ({ title, subtitle, form }) => {
  ...
  const handleFormSubmit = async (data) => {
    ...
    if (!session?.user.id) {
      ...
      router.push(
        searchParams.get("returnurl") ? searchParams.get("returnurl") : "/",
      );
      return;
    }
    if (!user?.user._id) {
  ...
  if (form == "password") {
    ...
    return (
      <motion.div
        className={`${isSubmitting && "pointer-events-none"} bg-white shadow-md w-full max-w-[500px] m-auto font-serif border- p-2 xs:p-5 mt-10 rounded-lg`}
      ...
      >
        <form
          action=""
          onSubmit={handleSubmit(handlePasswordChangeClick)}
          className=""
        >
          <motion.h1
          ...
          <div className="input_field_container flex flex-col gap-20 ">
            ...
            <Input
              ...
              validation={{
                ...
                pattern: {
                  value:
                    /(?=.*[0-9])((?=.*[a-z])|(?=.*[A-Z]))((?=.*\W)|(?=.*_))^[^ ]+$/,
                  message: "Password must contain letter, symbol & digit",
                },
              }}
            ...
            <Input
              ...
              validation={{
                validate: (fieldValue) => {
                  return fieldValue === newPassword || "Password didn't match";
                },
              }}
        ...
        <div className="changePwwithLink mt-10">
          <Link
            className=" hover:no-underline underline  block font-bold text-gray-500 transition duration-300"
            href="/auth/password-reset"
          >
            Change Password with an email Link?
          </Link>
        </div>
      </motion.div>
    );
  ...
  return (
    <motion.div
      className="w-full aa max-w-[600px] border- m-auto mt- font-serif border- p-2 xs:p-5 mt-10  rounded-lg"
      initial={{ opacity: 0, y: 5 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 1, delay: 1 }}
    >
...
export default isAuthenticated(UpdateDetails, {
  authenticateUserDetails: true,
});
