...
import {
  appendPopularArts,
  deletePopularArts,
  togglePopularArtsLike,
} from "@/redux/features/gallerySlice/popularSlice";
import {
  appendFollowingArts,
  deleteFollowingArts,
  toggleFollowingArtsLike,
} from "@/redux/features/gallerySlice/followingSlice";
import {
  appendRecentArts,
  deleteRecentArts,
  toggleRecentArtsLike,
} from "@/redux/features/gallerySlice/recentSlice";
...
const ArtCard = ({ item }) => {
  ...
  useEffect(() => {
    setArtData(item);
    setIsOwner(
      session?.user?.id === artData?.artist?.user ||
        session?.user?.id === artData?.artist?.user._id,
    );
  }, [item]);
  ...
  const toggleLike = async (productId) => {
    if (user?._id) {
      try {
        ...
        if (artData.likes.includes(user?._id)) {
          // already liked remove userid from array
          newLikes = newLikes.filter((id) => id !== user?._id);
        } else {
          //not liked add userid to the array
          newLikes.push(user?._id);
        }

        setArtData({ ...artData, likes: newLikes });
        dispatch(
          togglePopularArtsLike({ userId: user._id, productId: artData._id }),
        );
        dispatch(
          toggleFollowingArtsLike({ userId: user._id, productId: artData._id }),
        );
        dispatch(
          toggleRecentArtsLike({ userId: user._id, productId: artData._id }),
        );
        ...
        dispatch(
          toggleNewArtsLike({ userId: user._id, productId: artData._id }),
        );
    ...
    <>
      <div className="w-full min-w-[300px] sm;max-w-[400px] bg-white rounded-lg  darkk:bg-gray-800 darkk:border-gray-700">
        <Link
          ...
          className="block w-full aspect-[5/4] relative group"
        >
          <Image
            height={400}
            width={400}
          ...
          <div className="texts  flex items-end pb-4 px-3 justify-between absolute w-full bottom-0 left-0 h-[100px] from-transparent to-[rgba(0,0,0,.6)] from-0% bg-gradient-to-b text-gray-100 font-medium opacity-0 group-hover:opacity-100 rounded-lg transition duration-300">
            <p ref={artNameRef} className="title w-full flex-1 line-clamp-1">
              {artData?.name} &nbsp;
            </p>

            <span className="text-sm w-fit font-semibold text-gray-100 darkk:text-white">
              {artData?.price ? (
                <span className=" bg-green-700  p-1 px-2 rounded-full text-xs">
        ...
        <div className="px-1 pb-5 my-2.5 relative">
          <div className="head flex justify-between items-center">
            ...
            >
              ...
              <h6
                ref={artistNameRef}
                className="font-medium  w-full flex-1 h-fulltracking-tight text-gray-700 darkk:text-white capitalize"
              >
            ...
            <div className="flex items-center">
              ...
              >
                {!checkLiked(artData?.likes, user?._id) ? (
                  <BsHeart fontSize={"1.2rem"} fill="gray" />
                ) : (
                  <motion.span
                    className="block"
                ...
                <Menu as="div" className="relative z-50 inline-block text-left">
                  ...
                  >
                    <Menu.Items
                      ...
                      className="absolute right-0 mt-2  origin-top-right divide-y-8 divide-gray-800 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none"
                    >
