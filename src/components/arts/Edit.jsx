...
import { editPopularArts } from "@/redux/features/gallerySlice/popularSlice";
import { editRecentArts } from "@/redux/features/gallerySlice/recentSlice";
import { editFollowingArts } from "@/redux/features/gallerySlice/followingSlice";
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        dispatch(editPopularArts({ ...data, _id: productData._id }));
        dispatch(editRecentArts({ ...data, _id: productData._id }));
        dispatch(editFollowingArts({ ...data, _id: productData._id }));
        router.back();
      } else {
        setLoading(false);
        ...
        <div className="container my-12 w-full max-w-[600px] flex justify-center items-center mx-auto ">
          ...
          >
            ...
            <div className="description input_field_container">
              <label htmlFor="post-description" className="text-gray-500 -mb-2">
                Description
              </label>
              <textarea
                type="text"
            ...
            <div className=" image input_field_container ">
              ...
              <input
                id="uploadimage"
                className="p-5 hidden border-gray-300 hover:border-gray-400 transition border-2 rounded-md"
                type="file"
                label={"Upload Image"}
                required
