...
const Gallery = ({
  ...
  const { data, error, isLoading, isLoadingNewPage, hasMore } =
    useInfiniteScroll({
      url,
      data: reduxArts,
      dependencies,
  ...
  if (galleryData?.length == 0) {
    if (!hasMore) {
      return (
        <p className=" mt-40 leading-[100%]  max-w-[500px] mx-auto font-semibold text-2xl text-gray-400 text-center">
          <span>Oops! I am empty. </span>
          <span className="font-light text-base block  mt-3">
            🎨 Embrace the blank canvas: Nothing on display, Everything to
            create 🌟
          </span>
        </p>
      );
      ...
      <main className="gallery bg-white sm:m-5 mt-0 min-h-[100vh] p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-8  ">
        {galleryData?.length > 0 &&
          galleryData?.map((itm, idx) => {
            ...
            return (
              <ArtCard id={`artcardskeleton${idx}`} key={idx} item={itm} />
            );
          })}
      </main>
      <div className="loader mt-5 grid place-items-center">
        ...
        {!hasMore && (
          <div className="text-center p-5 text-gray-600 w-full opacity-50 ">
            <h1 className="mainHead  font-serif p-10  mb-5  ">
              {" "}
              {"🎨 Bravo!"}
