  ...
  useEffect(() => {
    const fetchExhibitions = async () => {
      try {
        ...
        const runningExhibitions = data.filter(
          (exhibition) => {
      ...
      {loading ? (
        ...
        {exhibitions.map((item, index) => (
          <div
            ...
            <div onClick={() => { openModal(index) }} className="flex items-center space-x-4 rtl:space-x-reverse pb-3 sm:pb-4">
              <div className="flex-shrink-0">
                <Image height={400} width={400} className="w-20 h-20 rounded-full object-contain bg-slate-300" src={item?.photo} alt=" image" />
              </div>
        ...
        <div className="flex items-center justify-end space-x-4 rtl:space-x-reverse pb-3 sm:pb-4">
          ...
          <Link href="/exhibition" className="inline-flex items-center hover:underline text-xs font-bold text-gray-900 cursor-pointer gap-2 ">
