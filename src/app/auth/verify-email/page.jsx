...
const Page = () => {
  ...
  const handleSendVerificationEmail = async () => {
    if (user?.user?.email) {
      try {
        toast.info("Working on delivering link! This may take few seconds.", {
          autoClose: 5000,
          toastId: "processing_email_message",
        });
  ...
  return (
    <div className="mx-auto w-fit mt-16 text-center max-w-[500px] p-2">
      <div className="icon grid place-items-center mb-5 text-gray-600">
        <span className=" animate-bounce   p-3 block w-fit rounded-full bg-[#00c72b33] outline outline-8  outline-[#00c72b11]">
          <MdEmail className="w-8 h-8" fill="#00c72b" />
        </span>
      </div>
      <h1 className="font-bold text-2xl xxs:text-3xl  text-gray-700">
      ...
      <p className="font- text-gray-700 mt-4">
        ...
        <br />
        {user?.user?.email && (
          <span className="font-bold text-base xxs:text-xl text-gray-600">{`"${user?.user?.email}"`}</span>
        )}
      </p>
      ...
      <p className="mt-8">
        ...
        <b className="text-gray-600">check your spam </b> folder.
      </p>
      <p className="mt-7">{` Still can't find the email? No problem.`}</p>
      ...
      >
        Resend &nbsp;
        <span className="hidden xxs:inline-block text-gray-200 hover:text-white">
          Verification
        </span>
        &nbsp; Link &nbsp;
      </button>
