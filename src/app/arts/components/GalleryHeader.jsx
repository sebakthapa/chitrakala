...
const GalleryHeader = ({ slug }) => {
    ...
    const handleLeftScroll = () => {
        console.log(categoriesContainerRef.current.scrollLeft + categoriesContainerRef.current.clientWidth, categoriesContainerRef.current.scrollWidth,categoriesContainerRef.current.scrollLeft )
        
        categoriesContainerRef.current.scrollLeft -= (categoriesContainerRef.current.clientWidth - 10)
    }
    const handleRightScroll = () => {
        console.log(categoriesContainerRef.current.scrollLeft + categoriesContainerRef.current.clientWidth, categoriesContainerRef.current.scrollWidth, )
        categoriesContainerRef.current.scrollLeft += (categoriesContainerRef.current.clientWidth - 10)
    }
        ...
        <header className="galleryNav p-5 bg-white flex sm:m-5 mb-0 justify-between">
            ...
            <div className="categoriesContainer bg-red-  max-w-[60%]  flex items-center" >

                {
                    ...
                    <li className={`scrollLeft -mr-4 pr-4 cursor-pointer grid place-items-center bg-gradient-to-r from-white via-white  to-transparent z-20 h-full text-gray-400 `} onClick={handleLeftScroll}>
                        <MdKeyboardArrowLeft className='bg-transparent' fontSize={"1.3rem"} />
                    </li>
                }

                <ul className='flex overflow-x-auto hide_scrollbar gap-1 items-center scroll-smooth' ref={categoriesContainerRef}>
                ...
                {
                    categoriesContainerRef?.current?.scrollWidth > categoriesContainerRef?.current?.clientWidth &&
                    <li className={`scrollLeft -ml-4 pl-4 cursor-pointer grid place-items-center bg-gradient-to-l from-white via-white  to-transparent z-20 h-full text-gray-400 `  } onClick={handleRightScroll}>
                        <MdKeyboardArrowRight className='bg-transparent' fontSize={"1.3rem"} />
                    </li>
                }
            </div>
