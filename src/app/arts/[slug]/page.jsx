...
const Page = ({ params: { slug } }) => {
  ...
  switch (slug) {
    case "popular":
      return (
        <Gallery
          url={"/api/products?sort=likesD"}
          reduxName={"popularArts"}
          dispatch={addPopularArts}
        />
      );
    case "following":
      return (
        <>
          {following?.length == 0 ? (
              ...
              <h1 className="mainHead  font-serif p-10  mb-5 bg-white ">
                ...
                <p className=" text-sm xxs:text-base  sm:text-[1.08rem]  leading-relaxed text-gray-700 antialiased py-5">
                  ...
                  <span className="text-sm ">
                    {
                      "Start your art journey by exploring and following talented creators! 🚀"
                    }
                  </span>
                  <br />
                  <Link
                    href="/arts/popular"
                    className="tex-sm text-blue-500 mt-2 block underline hover:no-underline"
                  >
                    Explore popular Arts
                  </Link>
                </p>
              </h1>
            </div>
            ...
            <Gallery
              ...
              url={`/api/products/following/${user?._id}?`}
              reduxName={"followingArts"}
    ...
    case "recent":
      return (
        <Gallery
          url={"/api/products?sort=newD"}
          reduxName={"recentArts"}
          dispatch={addRecentArts}
        />
      );
    default:
