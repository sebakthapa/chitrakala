  ...
  try {
    ...
    if (!token?.user.id) {
      return NextResponse.json(
        { message: "You must be logged in to get your details." },
        { status: 401 },
      );
    }
    ...
    if (token?.user.id !== id) {
      return NextResponse.json(
        { message: "You can't view other user's detail." },
        { status: 401 },
      );
    }
    ...
    if (!res)
      return new NextResponse(JSON.stringify({ error: "user doesn't exist" }), {
        status: 404,
      });
...
export const PATCH = async (request) => {
  try {
    ...
    if (token?.user.id != id) {
      return NextResponse.json(
        { message: "You can't update other user's details." },
        { status: 401 },
      );
    }
    ...
    if (updatedData.phone) {
      ...
      if (existingPhone?._id)
        return new NextResponse(
          JSON.stringify({
            field: "phone",
            message: "This phone is linked with another user.",
          }),
    ...
    if (updatedData.email) {
      ...
      if (existingEmail?._id) {
        return new NextResponse(
          JSON.stringify({
            field: "email",
            message: "This email is linked with another user.",
          }),
          { status: 403, statusText: "validation_error" },
        );
      }
