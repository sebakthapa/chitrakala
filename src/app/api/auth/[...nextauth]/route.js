...
import clientPromise from "../../../../services/mongodb.service";
...
import { MongoDBAdapter } from "../../../../configs/mongoDBAdapter";
    ...
    async jwt({ token, user, trigger, ...rest }) {
      if (trigger == "update") {
        ...
        const {
          ...
          emailVerified,
          _id: id,
        } = await Users.findById(token?.user.id).select(
    ...
    async signIn({ profile, account, metadata, email, ...props }) {
      if (account.provider == "email") {
        if (email?.verificationRequest) {
          ...
          try {
            await dbConnect();
            let res = await VerificationTokens.find(
            ...
            res?.forEach((object) => {
              ...
              const hourDiff =
                Math.abs(presentTime - expireTime) / 1000 / 60 / 60;
            ...
            if (validTokenLength > 1) {
              // validate frequent request only if at least one link is sent
              ...
              validTokens.forEach((diffHr) => {
                //diffHr = remaining validity time in hours / comes in asc
