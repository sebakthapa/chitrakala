...
export const GET = async (req) => {
    try {
        ...
        const userData = await UsersDetails.findOne({ user:userId }).populate("wishLists");

        if (userData && userData.wishLists) {
            return new NextResponse(JSON.stringify(userData.wishLists));
        }

        return new NextResponse(JSON.stringify({ error: "No wishLists to load" }), { status: 400 });
    ...
    try {
        ...
        if (userData) {
            ...
            if (!userData.wishLists.includes(productId)) {
                ...
                const newuserData = await UsersDetails.findOne({ user }).populate("wishLists");
                return new NextResponse(JSON.stringify(newuserData.wishLists));
            } else {
                return new NextResponse(JSON.stringify({ error: "Item already in wishLists" }), { status: 400 });
            }
        } else {
    ...
    try {
        ...
        if (userData) {
            ...
            if (itemIndex !== -1) {
                ...
                return new NextResponse(JSON.stringify({ error: "Item not found in wishLists" }), { status: 400 });
            }
        } else {
            return new NextResponse(JSON.stringify({ error: "User not found" }), { status: 404 });
        }
