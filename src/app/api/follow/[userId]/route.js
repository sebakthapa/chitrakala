...
export const PATCH = async (req, res) => {
  try {
    ...
    if (token?.user?.id != userId) {
      return NextResponse.json(
        { error: true, errorMessage: "You can only modify your followings" },
        { status: 401 },
      );
    }
    await dbConnect();

    let userRes = await Follows.findOne({
      userDetails: userDetailsId,
    }).populate("userDetails"); // this is followng array of user. (lists to whom has user followed)
    let artistRes = await Follows.findOne({
      userDetails: artistDetailsId,
    }).populate("userDetails"); // this is followers array (lists who has followed the artist) of artist who is being followed
    let isFollow;
    if (userRes == null) {
      const userFollow = new Follows({
        userDetails: userDetailsId,
        following: [artistDetailsId],
      });
    ...
    if (artistRes == null) {
      const artistFollow = new Follows({
        userDetails: artistDetailsId,
        followers: [userDetailsId],
      });
