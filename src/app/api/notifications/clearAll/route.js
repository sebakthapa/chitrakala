...
export const DELETE = async (req) => {
    try {
      ...
      // Validation checks
      if (!userId ) {
        return NextResponse.json({ message: "Invalid request. userId  is required.", error: null }, { status: 400 });
      }
      ...
      return NextResponse.json({ message: "Notification cleared successfully", data: user }, { status: 200 });
    } catch (error) {
      console.error(error);
